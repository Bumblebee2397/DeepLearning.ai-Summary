...
- Single neuron basically performs linear regression without applying any activation function. A single neuron in a neural network is also termed as perceptron.
- Basically a single neuron will calculate weighted sum of input(W(Transpose)*X+b) and then we can set a threshold to predict output in a perceptron. If weighted sum of input cross the threshold, perceptron fires and if not then the perceptron doesn't predict.
- Perceptron can take real values input or boolean values.
- Actually, when W(Transpose)⋅X+b=0 the perceptron outputs 0.
- Disadvantage of perceptron is that it can only output binary values and if we try to give it a small change in weight and bias then the perceptron can completely flip the output. We want a system which is capable of modifying the output slightly in accordance to a small change in weight and bias. Here comes sigmoid function in picture.
- If we change perceptron with a sigmoid function, then we can make slight change in output.
- If we apply sigmoid activation function then the single neuron will act as Logistic Regression.
...
- Deep learning is taking off for 3 reasons:
  1. Big data:
     - Using this image we can conclude:
       - ![](Images/11.png)
     - For small data, NN has similar performance when compared to Linear regression or SVM (Support vector machine).
     - For big data a small NN is better that SVM.
     - For big data a big NN is better that a medium NN is obviously better than a small NN.
     - Hopefully we have a lot of data because the world now is massively connected to a number of devices which is continously generating data.
       - Mobiles
...
- Mainly he is talking about how to do a logistic regression to make a binary classifier.
  - ![](Images/Binary classification.jpg)
